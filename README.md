...
For bare-metal deployment with CAPM3, Proxmox bootstrap options, and fake BMC lab options, see [docs/baremetal/README.md](docs/baremetal/README.md). For the cleaned-up ProxmoxBMC lab that includes every issue we hit and the validated fixes, see [docs/baremetal/proxmoxbmc-validated-lab.md](docs/baremetal/proxmoxbmc-validated-lab.md). For the final troubleshooting record from the real Sylva deployment, see [docs/baremetal/proxmox-sylva-deployment-issues-and-fixes.md](docs/baremetal/proxmox-sylva-deployment-issues-and-fixes.md). For the single-node Proxmox workload cluster plan for O-CU/O-DU, see [docs/baremetal/proxmox-single-node-workload-cluster.md](docs/baremetal/proxmox-single-node-workload-cluster.md). For the earlier Option B reference runbook, see [docs/baremetal/proxmoxbmc-option-b-runbook.md](docs/baremetal/proxmoxbmc-option-b-runbook.md). For adding Nephio as the intent-driven package automation layer above Sylva, see [docs/nephio-sylva-integration.md](docs/nephio-sylva-integration.md).
